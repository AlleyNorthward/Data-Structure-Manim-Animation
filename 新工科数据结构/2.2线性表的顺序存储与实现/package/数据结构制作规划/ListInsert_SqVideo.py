...
class ListInsert_SqVideo:
    def __init__(
        ...
        PATH_SQ4 = ASSETS_DIR / "展示代码" / "5ListInsert_Sq4.cpp"
        PATH_SQ5 = ASSETS_DIR / "展示代码" / "5ListInsert_Sq5.cpp"
        PATH_SQ6 = ASSETS_DIR / "展示代码" / "5ListInsert_Sq6.cpp"
        ...
        self.movingcode4 = MovingCode(PATH_SQ4).scale(0.7).to_edge(DOWN, buff = 0.2)
        self.movingcode5 = MovingCode(PATH_SQ5).set(width = self.scene.camera.frame_width - 3).move_to(ORIGIN)
        self.movingcode6 = MovingCode(PATH_SQ6).scale(0.8).move_to(ORIGIN)
    ...
    def second_scene(self):
        ...
        PATH_QUESTION = ASSETS_DIR / "思考疑问警告" / "问号1.svg"
        question = SVGMobject(PATH_QUESTION).scale(1.3)
        question.to_edge(DOWN, buff = 1)

        caption = Tex(
            *self.total_text.CAPTION6,
            tex_template = TexTemplateLibrary.ctex
        )
        caption.set_color(BLACK).scale(1.1)
        caption.next_to(t0, DOWN, buff = 0.5)

        caption2 = Tex(
            *self.total_text.CAPTION7,
            tex_template = TexTemplateLibrary.ctex
        )
        caption2.set_color(BLACK).scale(1.1)
        caption2.next_to(t0, DOWN, buff = 0.5)

        caption3 = Tex(
            self.total_text.CAPTION8,
            tex_template = TexTemplateLibrary.ctex
        )
        caption3.set_color(BLACK).scale(0.4)
        caption3.next_to(nodes[4], DOWN, buff = 0.05)

        caption4 = Tex(
            self.total_text.CAPTION9,
            tex_template = TexTemplateLibrary.ctex
        )
        caption4.set_color(BLACK).scale(0.6)
        caption4.next_to(nodes[3], UP, buff = 0.05)
        ...
        t0.get_entries()[4].save_state()

        self.scene.play(
            t0.get_entries()[4].animate.set_color(RED).scale(1.4)
        )
        self.scene.wait()
        self.scene.play(
            Wiggle(t0.get_entries()[5]),
            Restore(t0.get_entries()[4])
        )

        t0.get_entries()[0].save_state()
        t0.get_entries()[6].save_state()
        self.scene.play(
            AnimationGroup(
                t0.get_entries()[0].animate.scale(2).set_color(MAROON),
                t0.get_entries()[6].animate.scale(1.25).set_color(TEAL),
            ),
        )
        
        self.scene.play(
            Restore(t0.get_entries()[0]),
            Restore(t0.get_entries()[6]),
            FadeOut(question),
        )
        self.scene.play(
            AnimationGroup(
        ...
        self.scene.play(
            AnimationGroup(
                Write(caption),
                t0.get_entries()[1].animate.set_value(4),
                t0.get_entries()[7].animate.set_value(4),
                single_node.animate.move_to([nodes[4].get_center()[0], single_node.get_center()[1], 0])
            )
        )
        self.scene.wait(2)
        ...
        self.scene.wait()
        #此时在视频第20s处, 基本已经修改.
        # 不要忘了Restore, 修改cpation, i的值, 插入节点6

        self.scene.play(
            FadeOut(c),
            ReplacementTransform(caption, caption2),
            t0.get_entries()[1].animate.set_value(3),
            t0.get_entries()[7].animate.set_value(3),
            single_node.animate.move_to([nodes[3].get_center()[0], single_node.get_center()[1], 0])
        )
        self.scene.wait()
        self.scene.play(
            Succession(
        ...
        self.scene.play(
            AnimationGroup(
                FadeOut(c2),
                AnimationGroup(
                    ReplacementTransform(single_node, nodes[3]),
                    nodes[3].animate.change_info("6")
                ),
            ),
        )
        ...
        caption2_copy = caption2.copy()
        self.scene.play(
            caption2_copy.animate.move_to(reference_rec.get_center()),
            FadeOut(caption2)
        )

        self.scene.wait()
        self.scene.play(
            FadeOut(caption2_copy)
        )
        ...
        self.scene.wait()
        self.scene.play(
            caption4.animate.move_to([nodes[1].get_center()[0], caption4.get_center()[1], 0])
        )
        self.scene.wait()
        self.scene.play(
            caption4.animate.move_to([nodes[0].get_center()[0], caption4.get_center()[1], 0])

        )
        self.scene.wait()

        self.scene.play(
            self.movingcode3.transform_remark_rectangle(moving_rect2,7)
        )
        self.scene.play(
            FocusOn(self.movingcode3[2][6][6])
        )
        ...
        self.scene.play(
            AnimationGroup(
                FadeOut(self.movingcode3),
                FadeOut(t0),
                FadeOut(nodes),
                FadeOut(caption3),
                FadeOut(caption4),
                FadeOut(moving_rect2),
            )
        )
        self.scene.clear()
    ...
    def third_scene(self):
        self.scene.play(
            FadeIn(self.movingcode1)
        )
        
        self.scene.play(
            self.movingcode1.animate.move_to(ORIGIN)
        )

        moving_rect_third = self.movingcode1.get_rect(2,4)
        self.scene.play(
            FadeIn(moving_rect_third)
        )

        self.scene.play(
            FocusOn(self.movingcode1[2][1][7])
        )

        self.scene.wait()
        self.scene.play(
            FocusOn(self.movingcode1[2][1][11])
        )

        self.scene.wait(1.7)

        caption = Tex(self.total_text.CAPTION10, tex_template = TexTemplateLibrary.ctex)
        caption.set_color(BLACK)
        caption.scale(1.3)
        caption.next_to(self.movingcode1, DOWN, buff = 0.3)
        ...
        self.scene.play(
            FocusOn(self.movingcode1[2][1][18])
        )
        self.scene.wait(2)

        self.scene.play(
            self.movingcode1.transform_remark_rectangle(moving_rect_third, 5, 9),
            FadeOut(caption)
        )
        self.scene.wait(3.2)

        self.scene.play(
            FocusOn(self.movingcode1[2][4][15])
        )

        self.scene.wait(2)

        self.scene.play(
            FocusOn(self.movingcode1[2][5][31])
        )

        self.scene.wait()

        PATH_THINK = ASSETS_DIR / "思考疑问警告" / "搜索问号.svg"
        think = SVGMobject(PATH_THINK).scale(0.4)
        think.next_to(self.movingcode1[2][5][31], DOWN, buff = 0.1)
        ...
        self.scene.play(
            FadeOut(think),
            FocusOn(self.movingcode1[2][5][45])
        )
        self.scene.wait()

        self.scene.play(
            FadeOut(moving_rect_third),
        )

        self.scene.play(
            self.movingcode1.animate.to_edge(UP, buff = 0.07)
        )
    ...
    def fourth_scene(self):
        ...
        self.scene.wait(5)
        self.scene.play(
            FadeIn(self.movingcode4)
        )

        self.scene.play(
            self.movingcode4.animate.next_to(self.movingcode1, DOWN, buff = 0.1)
        )

        self.scene.play(
            FocusOn(self.movingcode4[2][1][9])
        )

        self.scene.play(
            FocusOn(self.movingcode4[2][1][25])
        )

        self.scene.play(
            FadeOut(self.movingcode4),
            self.movingcode1.animate.move_to(ORIGIN),
        )

        self.scene.play(
            ReplacementTransform(self.movingcode1, self.movingcode5)
        )
        self.scene.play(
            FocusOn(self.movingcode5[2][1][6])
        )

        self.scene.play(
            ReplacementTransform(self.movingcode5, self.movingcode6)
        )

        self.scene.wait(8)
        self.scene.play(
            FadeOut(self.movingcode6)
        )
        self.scene.wait(0.2)