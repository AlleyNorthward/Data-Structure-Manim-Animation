from manim import (
    Tex, TexTemplateLibrary, VGroup, SVGMobject,
    FadeIn, Create, Write, FadeOut, GrowFromCenter, Circumscribe, AnimationGroup, Succession, Wiggle,
    BLACK, YELLOW_E, GRAY_D, BLUE_D, ORANGE, GREEN_D, RED_D,
    UP, DOWN, RIGHT, UL, LEFT, ORIGIN, DL,
    Scene,
)
...
from .VideoChapter import VideoChapter
from .ListErase_SqVideo import ListErase_SqVideo
...
class SqListVideo:
    ...
    def __init__(
        ...
        self.chapter_video = VideoChapter(scene)
        self.scene = scene
    ...
    def insert_animation(self):
        ...
        insert_video.third_scene()
        insert_video.fourth_scene()
    
    def erase_animation(self):
        erase_video = ListErase_SqVideo(self.scene)
        erase_video.first_scene()
        erase_video.second_scene()
        erase_video.third_scene()
        erase_video.fourth_scene()
        erase_video.fifth_scene()

    def find_animation(self):
        find_video = ListErase_SqVideo(self.scene)
        find_video.sixth_scene()

    def traverse_animation(self):
        traverse_video = ListErase_SqVideo(self.scene)
        traverse_video.seven_scene()

    def broaden_animation(self):
        broaden_video = ListErase_SqVideo(self.scene)
        broaden_video.eight_scene()

    def end_animation(self):
        end_video = ListErase_SqVideo(self.scene)
        end_video.nine_scene()
        
    def chapter_animation(self):
        # 这是后续追加的, 其实应该放到上面方法中的, 但由于不好理清, 所以专门写个函数
        # 生成章节动画, 之后拖过剪辑手段添加进去.
        self.chapter_video.set_tex(self.total_text.chapter8)
        self.chapter_video.create()
